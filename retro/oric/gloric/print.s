...
_SimplePrint
	...
	ldy #0
	lda (sp),y
	sta SimplePrint_read+1
	iny
	lda (sp),y
	sta SimplePrint_read+2

	; Initialise display adress
	; this uses self-modifying code
	; (the $0123 is replaced by display adress)
	lda #<DISPLAY_ADRESS
	sta SimplePrint_write+1
	lda #>DISPLAY_ADRESS
	sta SimplePrint_write+2
...
SimplePrint_write
	...
	inx
	jmp SimplePrint_loop_char  

	; Finished !
SimplePrint_end_loop_char
	rts
...
_AdvancedPrint
	...
	ldy #4
	lda (sp),y
	sta AdvancedPrint_read+1
	iny
	lda (sp),y
	sta AdvancedPrint_read+2
...
AdvancedPrint_write
	...
	; Next character, and loop
	inx
	jmp AdvancedPrint_loop_char  

	; Finished !
AdvancedPrint_end_loop_char
	rts
