...
pgn = open(f"C:\\Users\\tbpk7658\\Documents\\repos\\hello-world\\web\\chess\\games_{Account}.pgn", encoding="ansi")
...
whiteGameTree.headers["White"] = Account
...
blackGameTree.headers["Black"] = Account
...
dictWhiteOpening = []
for var in whiteGameTree.variations:
    runThrough (var, 24, dictWhiteOpening)

with open(f"opening_wt_{Account}.js", "w") as ficout:
    ficout.write ("const openingBookWhite = "+json.dumps(dictWhiteOpening, indent = 4))
dictBlackOpening = []
for var in blackGameTree.variations:
    runThrough (var, 24, dictBlackOpening)

with open(f"opening_bk_{Account}.js", "w") as ficout:
    ficout.write ("const openingBookBlack = "+json.dumps(dictWhiteOpening, indent = 4))
