...
class MidiTrack:
    ...
    def addEvent(self, delta_t, event):
        self.events.append(delta_t)
...
if __name__ == "__main__":
    ...
    mf = MidiFile()
    mf.ticksPerQuarterNote = 120
    
    aTrack = MidiTrack(0)
    
    aDTime = DeltaTime(aTrack)
    aDTime.time = 0
    
    aEvent = MidiEvent(aTrack)
    aEvent.time = 0
    aEvent.type = "SEQUENCE_TRACK_NAME"
    aEvent.channel = None
    aEvent.pitch = None
    aEvent.velocity = None
    aEvent.data = 'control'
    aTrack.addEvent(aDTime, aEvent)

    aEvent = MidiEvent(aTrack)
    aEvent.time = 0
    aEvent.type = "TIME_SIGNATURE"
    aEvent.channel = None
    aEvent.data = '\x02\x01\x12\x08'
    aTrack.addEvent(aDTime, aEvent)

    aEvent = MidiEvent(aTrack)
    aEvent.time = 0
    aEvent.type = "SET_TEMPO"
    aEvent.channel = None
    aEvent.data = '\x06[\x9a'
    aTrack.addEvent(aDTime, aEvent)
...
#    aTrack.channels[2].noteOn(36, 960, 98) #pitch, time, velocity
#    aTrack.channels[2].noteOff(36, 51) #time, velocity
